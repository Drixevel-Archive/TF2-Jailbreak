...
public Action:AdminDebugger(client, args)	//Terrible debugger, just did it to figure out what's fucking up where.
{
	if (j_Enabled)
	{
...
{
	if (!AlreadyMuted(client) && !Client_HasAdminFlags(client, ADMFLAG_RESERVATION) && !g_IsMuted[client])
	{
		Client_Mute(client);
