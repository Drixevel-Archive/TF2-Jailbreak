...
#define PLUGIN_NAME     "[TF2] Jailbreak - Bans"
...
DisplayRageBanMenu(Client, ArraySize)
{
	if (ArraySize == 0)
	{
		CPrintToChat(Client, "%s %t", CLAN_TAG, "No Targets");
	}
	else
	{
		...
		case MenuAction_Select:
			{
				...
				if (debugging) CPrintToChat(param1, "%s %t", CLAN_TAG, "Ready to Guard Ban", sInfoString);
			}
	}
}
...
public CP_Callback_CheckBan(Handle:owner, Handle:hndl, const String:error[], any:stringPack)
{
	...
	else
	{
		...
		CShowActivity2(iAdminIndex, CLAN_TAG, "%t", "Rage Ban", sTargetName);
...
public Action:Command_Offline_Ban(client, args)
{
	...
	if (bAuthIdNativeExists)
	{
		...
		CReplyToCommand(client, "%s %t", CLAN_TAG, "Banned AuthId", sAuthId);
	}
	else
	{
		CReplyToCommand(client, "%s %t", CLAN_TAG, "Feature Not Available");
	}
...
public Action:Command_Offline_Unban(client, args)
{
	...
	if (bAuthIdNativeExists)
	{
		...
		CReplyToCommand(client, "%s %t", CLAN_TAG, "Unbanned AuthId", sAuthId);
	}
	else
	{
		CReplyToCommand(client, "%s %t", CLAN_TAG, "Feature Not Available");
...
public Action:Command_RageBan(client, args)
{
	...
	if (iArraySize == 0)
	{
		CReplyToCommand(client, "%s %t", CLAN_TAG, "No Targets");
		return Plugin_Handled;
	}
	...
	if (!args)
	{
		...
		{
			CReplyToCommand(client, "%s %t", CLAN_TAG, "Feature Not Available On Console");
		}
		return Plugin_Handled;
	}
	else
	{
		CReplyToCommand(client, "%s Usage: sm_rageban", CLAN_TAG);
	}
...
public OnConfigsExecuted()
{
	...
	if (mysql_cvar)
	{
		...
		{
			LogError("SQL configuration value '%s' could not be found, please check database configuration file.", sDatabaseDriver);
		}
	}
	else
	{
		...
		{
			LogError("SQL clientprefs entry could not be found, please check database configuration file.");
		}
	}
}
		...
		case MenuAction_Select:
			{
				...
				{
					CPrintToChat(param1, "%s %t", CLAN_TAG, "Already Guard Banned", iTargetIndex);
				}
			}
	}
}
		...
		case MenuAction_Select:
			{
				...
				{
					CPrintToChat(param1, "%s %t", CLAN_TAG, "Player no longer available");
				}
				else if (!CanUserTarget(param1, target))
				{
					CPrintToChat(param1, "%s %t", CLAN_TAG, "Unable to target");
				}
				else
				{
	...
	if (client && IsClientInGame(client))
	{
		...
		if (StrEqual(cookie, "1")) 
		{
			if (GetClientTeam(client) == _:TFTeam_Blue)
			{
				...
				CPrintToChat(client, "%s %t", CLAN_TAG, "Enforcing Guard Ban");
			}		
		}
	}
}
...
public Action:Command_LiveUnban(client, args)
{
	if (args < 1)
	{
		CReplyToCommand(client, "%s Usage: sm_unctban <player>", CLAN_TAG);
	}
	else
	{
		...
		else
		{
			...
			{
				CReplyToCommand(client, "%s %t", CLAN_TAG, "Cookie Status Unavailable");
			}
		}	
	}
...
Remove_CTBan(adminIndex, targetIndex, bExpired=false)
{
	...
	if (banFlag)
	{
		...
		if (!bExpired)
		{
			CShowActivity2(adminIndex, CLAN_TAG, "%t", "Guard Ban Removed", targetIndex);
		}
		else
		{
			CShowActivity2(adminIndex, CLAN_TAG, "%t", "Guard Ban Auto Removed", targetIndex);
		}
...
public Action:Command_LiveBan(client, args)
{
	if (args < 1)
	{
		CReplyToCommand(client, "%s Usage: sm_banguard <player> <time> <reason>", CLAN_TAG);
	}
	else
	{
		...
		else
		{
			if (target_list[0] && IsClientInGame(target_list[0]))
			{
				if (AreClientCookiesCached(target_list[0]))
				{
					...
					if (banFlag)
					{
						CReplyToCommand(client, "%s %t", CLAN_TAG, "Already Guard Banned", target_list[0]);
					}
					else
					{
...
PerformCTBan(client, adminclient, banTime=0, reason=0, String:manualReason[]="")
{
	...
	if (banTime > 0)
	{
		CShowActivity2(adminclient, CLAN_TAG, "%t", "Temporary Guard Ban", client, banTime);
		PushArrayCell(TimedBanLocalList, client);
	...
	{
		CShowActivity2(adminclient, CLAN_TAG, "%t", "Permanent Guard Ban", client);	
	}
}
...
public Action:Command_IsBanned(client, args)
{
	if (args < 1)
	{
		CReplyToCommand(client, "%s Usage: sm_isbanned <player>", CLAN_TAG);
		return Plugin_Handled;
	}
	...
	else
	{
		if (target_list[0] && IsClientInGame(target_list[0]))
		{
			if (AreClientCookiesCached(target_list[0]))
			{
				...
				if (banFlag)
				{
					...
					{
						CReplyToCommand(client, "%s %t", CLAN_TAG, "Permanent Guard Ban", target_list[0]);
					}
					else
					{
						CReplyToCommand(client, "%s %t", CLAN_TAG, "Temporary Guard Ban", target_list[0], LocalTimeRemaining[target_list[0]]);
					}
				}
				else
				{
					CReplyToCommand(client, "%s %t", CLAN_TAG, "Not Guard Banned", target_list[0]);
				}
			}
			else
			{
		...
		{
			CReplyToCommand(client, "%s %t", CLAN_TAG, "Unable to target");
		}				
	}
	return Plugin_Handled;
}
